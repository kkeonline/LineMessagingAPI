...
//LINE.setUserID(LINE_USER);
//
//or call line begin and set only token then set userID later
//LINE.begin(LINE_TOKEN);
//LINE.setUserID(LINE_USER);
//
//or just set the token and set userID later before call
//LINE.setToken(LINE_TOKEN);
  ...
  if(wifiConnected()) {
	...
	//or
    //LINE.setUserID(LINE_USER);
    //LINE.push("Test \"Message\" from\t'ESP8266'\r\n\\Running IP\\ " + WiFi.localIP().toString())) {
    //
	//or
    //LINE.push("Test \"Message\" from\t'ESP8266'\r\n\\Running IP\\ " + WiFi.localIP().toString()), LINE_USER) {
  }
}
...
void loop(){
  delay(20000);
    // get the return code
    if (LINE.push("Support 'Single quote\' \"Double quote\" \\Back slash\\ \ttab\t !@#$%^&*()_+-=;:.,/")) {
